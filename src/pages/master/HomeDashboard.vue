  ...
  <div class="h-screen flex overflow-hidden">
    ...
    <div class="sidebar" :class="{ 'block': settings.openSidebar, 'hidden': !settings.openSidebar }">
      <div class="sidebar-logo">
        <div class="flex items-cebg-gray-900nter" @click="gotoHome">
    ...
    <!-- Main-->
    <div class="flex-1 flex flex-col w-full  dark:bg-gray-900 relative overflow-hidden">

      <div class="overflow-y-auto">
        <div class="text-m mt-7 ml-8 dark:text-white">
          <router-link to="/"><strong>Anasayfa</strong></router-link>
          <router-link v-for="(crumb, index) in crumbs" :to="crumb.route" :key="index"><strong>{{ crumb.text }}</strong>
          </router-link>
        </div>
        <div class=" dark:bg-gray-900 flex-1 p-[20px] flex flex-col ">
          <div class="border border-gray-300 dark:border-gray-700 rounded-md p-[20px] flex-1">
            <router-view />
          </div>
        </div>
        <div class="p-4 text-center text-secondary-700 dark:bg-gray-700 dark:text-secondary-200">
          <label> © 2024 | </label>
          <a class="dark:text-secondary-400" href="#">Vita Systems</a>
        </div>
      </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    // sayfa yenilenmeden degişmiyor anlık olarak degişmesi
    watchEffect(() => {
...
.sidebar {
  @apply w-[300px] h-full bg-blue-600;
